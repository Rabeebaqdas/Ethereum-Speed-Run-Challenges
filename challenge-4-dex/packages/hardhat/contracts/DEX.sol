...
contract DEX {
	...
	function init(uint256 tokens) public payable returns (uint256) {
		require(msg.value > 0, "DEX: insufficient funds");
	...
	function tokenToEth(uint256 tokens) public returns (uint256) {
		require(tokens > 0, "Insufficient tokens sent");
		uint256 token_reserve = token.balanceOf(address(this));
	...
	function deposit() public payable returns (uint256) {
		...
		require(
			token.balanceOf(msg.sender) >= token_amount,
			"User dont have enough baloon tokens"
		);
	...
	function withdraw(uint256 amount) public returns (uint256, uint256) {
		...
		require(liquidity[msg.sender] >= eth_amount, "amount can't be 0 ");
